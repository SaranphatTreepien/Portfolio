...
// 👇 แก้ไขจุดที่ 1: รับ { slug } เข้ามาใน component
export default function ExperienceEditor({ slug }) {
  ...
  useEffect(() => {
    setMounted(true);
    // ตรวจสอบว่ามี slug ก่อนค่อยดึงข้อมูล
    if (slug) {
        fetchProjects();
    }
  }, [slug]); // เพิ่ม slug เป็น dependency

  // 👇 แก้ไขจุดที่ 2: ดึงข้อมูลเฉพาะ Slug นี้
  const fetchProjects = async () => {
    try {
        ...
        // ถ้า API คืนค่าเป็น Array ก็ใช้ได้เลย ถ้า error ให้เป็น array ว่าง
        ...
        {projects.length === 0 ? (
            ...
            {projects.map((item, index) => (
                ...
                <div className="absolute top-4 right-4 z-20 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    ...
                    {/* ปุ่ม Edit/Delete จะติด Auth Check อัตโนมัติ */}
