...
export default function ResumePage() {
    ...
    const [toastMessage, setToastMessage] = useState(""); // สำหรับข้อความ Toast
    ...
    const sendEmail = async () => {
        ...
        try {
            ...
            if (data.success) {
                setToastMessage("📧 ส่งอีเมลสมัครงานสำเร็จแล้ว!");
                setSuccess(true);
                setTimeout(() => {
                    setSuccess(false);
                    setToastMessage("");
                ...
                alert("❌ ส่งเมลไม่สำเร็จ: " + data.error);
            }
        } catch (err) {
            console.error(err);
    ...
    const handleDownload = () => {
        ...
        setToastMessage("📄 ไฟล์ resume.pdf ถูกบันทึกแล้ว!");
        setSuccess(true);
        setTimeout(() => {
            setSuccess(false);
            setToastMessage("");
            ...
            <AnimatePresence>
                {toastMessage && (
                    <motion.div
                        ...
                        className="fixed bottom-10 left-1/2 transform -translate-x-1/2 bg-green-500 text-white px-6 py-3 rounded-xl shadow-lg z-50"
                    >
                        {toastMessage}
                    </motion.div>
                )}
                ...
                >
                    ...
                    >
                        ...
                        <button type="submit"
                            ...
                            disabled={loading}
                        >
                            {loading ? "Sending..." : "Send Resume Email"}
                        </button>
                    </form>
                </motion.div>
            )}
                    ...
                    >
                        ...
                        >
                            <p className="text-gray-800 mb-4 text-lg font-medium">
                                คุณกำลังจะดาวน์โหลดไฟล์ <span className="font-bold">resume.pdf</span>
                            </p>
                            <p className="text-gray-500 mb-6">ต้องการดำเนินการต่อหรือไม่?</p>
                            <div className="flex justify-center gap-4">
                                ...
                                <button
                                    className="px-4 py-2 bg-[#7edad2] text-white rounded-lg hover:bg-[#79d3cc] transition"
                                    onClick={() => {
                                        handleDownload();
            ...
            <AnimatePresence>
                {showSendAlert && (
                    ...
                    >
                        ...
                        >
                            <p className="text-gray-800 mb-4 text-lg font-medium">
                                คุณต้องการส่ง Resume ไปที่ <span className="font-bold"> {form.receiverEmail}</span> หรือไม่?
                            </p>
                            <div className="flex justify-center gap-4">
                                <button
                                    className="px-4 py-2 bg-gray-300 rounded-lg hover:bg-gray-400 transition"
